...
app.get('/api/logout', (req, res) => {
    if (req.session.user && req.session.user.loginTime) {
        ...
        const duration = (Date.now() - loginTime) / 1000; // Duração em segundos
        console.log(`[MONITOR] Usuário ${req.session.user.email} saiu em: ${new Date().toISOString()} | Duração da sessão: ${duration.toFixed(2)} segundos.`);
    }
    req.session.destroy();
    res.redirect('/auth/login.html');
});
...
app.post('/api/log-exit', async (req, res) => {
    if (req.session.user && req.session.user.loginTime) {
        ...
        console.log(`[MONITOR] Usuário ${req.session.user.email} fechou a página em: ${new Date().toISOString()} | Duração da sessão: ${duration.toFixed(2)} segundos.`);
    }
